...
* description: Stepper Motor contorller + reading kepad + LCD
*********************************************************************************************************
*/
...
char KEYPAD_PIN_Scan(void);
void Motor_Init();
void Motor_move_clockwise(int degree);
void Motor_move_anticlockwise(int degree);
int degree(unsigned char word[3]);
void DO_K();
...
void LCD_PIN_Init(void){  
  ...
  GPIOA->MODER &= ~(GPIO_MODER_MODER1);	
	...
	//GPIOB (3,4,5,8,9,10,11,12,13,14,15) 
  ...
  for (i = 12,j=0; j<4; i-=4,j++) {
	...
	void DO_K()
 	{	
		 ...
		 word[0]=0x7F;word[1]=0x7F;word[2]=0x7F;
		 ...
		 while(1){
   			 ...
   			 k = KEYPAD_PIN_Scan();
				 ...
				 if(k=='A') 
				 {
					 Motor_move_clockwise(degree(word));
					 k=' ';
				 }
				 
				 if(k=='B') 
				 {
					 Motor_move_anticlockwise(degree(word));
					 k=' ';
				 }
				 
				 if((k!=0x7F)&&(word[0]==0x7F))
				 {
				 word[0]=k;
				 if (k=='#')word[0]=0x7F;
				 LCD_DisplayString(word);
         while(k!=0x7F) k = KEYPAD_PIN_Scan();			 
				 }
				 
				 if((k!=0x7F)&&(word[1]==0x7F))
				 {
				 word[1]=k;
				 if (k=='#'){word[0]=0x7F; word[1]=0x7F;}
				 LCD_DisplayString(word); 
				 while(k!=0x7F) k = KEYPAD_PIN_Scan();					 
				 }
				 
				 if((k!=0x7F)&&(word[2]==0x7F))
				 {
				 word[2]=k;
				 if (k=='#'){word[1]=0x7F; word[2]=0x7F;}
				 LCD_DisplayString(word);
				 while(k!=0x7F) k = KEYPAD_PIN_Scan();
				 }
   ...
   GPIOC->OSPEEDR &= ~(GPIO_OSPEEDER_OSPEEDR0);
	...
	void KEYPAD_PIN_Init(void){
		...
		//GPIOB (C1, C2, C3 and C4 of the Keypad)(8,13,14,15)- Inputs/Pulldown mode
	 ...
	 GPIOB->PUPDR &= ~(GPIO_PUPDR_PUPDR15_1);
		...
		//GPIOA (R1, R2, R3 and R4) (5,11,12,15)
	...
	char KEYPAD_PIN_Scan(void)
	{ 
			if((GPIOB->IDR & GPIO_OTYPER_IDR_8) == 0)
			{
					if ((GPIOB->IDR & GPIO_OTYPER_IDR_8)==0)
					{
						GPIOA->ODR |= GPIO_OTYPER_ODR_5;
						if ((GPIOB->IDR & GPIO_OTYPER_IDR_8))
						{
							GPIOA->ODR &= ~GPIO_OTYPER_ODR_5;
							return '1';
						}
      ...
      if((GPIOB->IDR & GPIO_OTYPER_IDR_13) == 0)
			{
					if ((GPIOB->IDR & GPIO_OTYPER_IDR_13)==0)
					{
						GPIOA->ODR |= GPIO_OTYPER_ODR_5;
						if ((GPIOB->IDR & GPIO_OTYPER_IDR_13))
						{
							GPIOA->ODR &= ~GPIO_OTYPER_ODR_5;
							return '2';
						}
      ...
      if((GPIOB->IDR & GPIO_OTYPER_IDR_14) == 0)
			{
					if ((GPIOB->IDR & GPIO_OTYPER_IDR_14)==0)
					{
						GPIOA->ODR |= GPIO_OTYPER_ODR_5;
						if ((GPIOB->IDR & GPIO_OTYPER_IDR_14))
						{
							GPIOA->ODR &= ~GPIO_OTYPER_ODR_5;
							return '3';
						}
      ...
      if((GPIOB->IDR & GPIO_OTYPER_IDR_15) == 0)
			{
					if ((GPIOB->IDR & GPIO_OTYPER_IDR_15)==0)
					{
						GPIOA->ODR |= GPIO_OTYPER_ODR_5;
						if ((GPIOB->IDR & GPIO_OTYPER_IDR_15))
						{
							GPIOA->ODR &= ~GPIO_OTYPER_ODR_5;
							return 'A';
						}
...
void Motor_move_clockwise(int degree){
	...
	int j,i,step,rotation;

		rotation = degree/60; //one sequnce
		degree = degree-(rotation*60); //left degree to do
	
			for(j=0; j<rotation;j++){				 
				for(step = 4; step >=0; step--){
            for(i = 0; i<4000; i++); //A short delay
						GPIOC->ODR = ((fullstep[step])<<0);		 
					  }
       }
//			
//			for(step = degree; step >=0; step--){
//			for(step = 4; step >=0; step--){
//            for(i = 0; i<4000; i++); //A short delay
//						GPIOC->ODR = ((fullstep[step])<<0);
//					  step = step-3;
